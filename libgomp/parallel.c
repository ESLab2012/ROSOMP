...
int strategy = -1;
void omp_set_strategy(int temp){
  strategy = temp;
}
ialias(omp_set_strategy)

int executor_num = -1;
void omp_set_executor_thread_num(int temp){
  executor_num = temp;
}
ialias(omp_set_executor_thread_num)
...
void omp_set_tid_map(long int tid, int executor_id){
  insert(tid, executor_id);
}
ialias(omp_set_tid_map)

#define max_executor_num  10
int priority[max_executor_num];
void omp_set_priority(int executor_id, long int pri){
  priority[executor_id] = pri;
}
ialias(omp_set_priority)

int omp_get_priority(){
...
void omp_synchronization(int priority){
...
void omp_set_wait_for_work_flag(bool flag){
  wait_for_work_flag = flag;
}
ialias(omp_set_wait_for_work_flag)

bool omp_get_wait_for_work_flag(){
  return wait_for_work_flag;
}
ialias(omp_get_wait_for_work_flag)

bool wait_for_work_flag_picas;
void omp_set_wait_for_work_flag_picas(bool flag){
  wait_for_work_flag_picas = flag;
}
ialias(omp_set_wait_for_work_flag_picas)

bool omp_get_wait_for_work_flag_picas(){
  return wait_for_work_flag_picas;
}
ialias(omp_get_wait_for_work_flag_picas)
