...
bool
Executor::get_next_ready_executable_picas(AnyExecutable & any_executable)
{
  bool success = get_next_ready_executable_from_map_picas(any_executable, weak_groups_to_nodes_);
  return success;
}

bool
Executor::get_next_ready_executable_from_map_picas(
  AnyExecutable & any_executable,
  const rclcpp::memory_strategy::MemoryStrategy::WeakCallbackGroupsToNodesMap &
  weak_groups_to_nodes)
{
  ...
  memory_strategy_->get_next_timer_picas(any_executable, weak_groups_to_nodes);
  if (any_executable.timer) {
    highest_priority = any_executable.timer->callback_priority;
  }

  memory_strategy_->get_next_subscription_picas(any_executable, weak_groups_to_nodes);
  ...
  memory_strategy_->get_next_service_picas(any_executable, weak_groups_to_nodes);
  ...
  memory_strategy_->get_next_client_picas(any_executable, weak_groups_to_nodes);
  ...
  memory_strategy_->get_next_waitable_picas(any_executable, weak_groups_to_nodes);
...
// ROSOMP Function
// This function is used to wake up the sleeping thread because wait_set is empty
void
...
// ROSOMP Function
// This function is used to arbitrate the scheduling control of the executor thread to be obtained by the ROSOMP Executor or the OMP Scheduler
// A return value of 0 indicates that the ROSOMP Executor has gained control
// A return value of 1 indicates that the OMP Scheduler has gained control
int 
Executor::Call_Comparison(){
  uint64_t start = get_clocktime();
  int res;
  int omp_queue_highest_pri = omp_queue_empty_check();
  if(omp_queue_highest_pri == -1){
    // omp_queue is empty
    res = 0;
  }else{
    // omp_queue is non-empty
    // the priorities of sub-callbacks in omp_queue is higher than that of wait_set
    res = 1;
  }
  uint64_t end = get_clocktime();
  printf("Call_Comparison : |%ld|%ld|%ld|\n", start, end, end-start);
  return res;
}

// ROSOMP Function
int 
Executor::Call_Comparison_picas(AnyExecutable & any_executable){
  int res;
  bool success = get_next_ready_executable_picas(any_executable);
  if(success){
    // wait_set is non-empty
    int omp_queue_highest_pri = omp_queue_empty_check();
    if(omp_queue_highest_pri == -1){
      // omp_queue is empty
      if (any_executable.timer) {
      ...
      // omp_queue not empty
      int wait_set_highest_pri = 0;
      if (any_executable.timer) {
        wait_set_highest_pri = any_executable.timer->callback_priority;
      }else if (any_executable.subscription) {
        wait_set_highest_pri = any_executable.subscription->callback_priority;
      }else if (any_executable.service) {
        wait_set_highest_pri = any_executable.service->callback_priority;
      }else if (any_executable.client) {
        wait_set_highest_pri = any_executable.client->callback_priority;
      }else if (any_executable.waitable) {
        wait_set_highest_pri = any_executable.waitable->callback_priority;
      }
      if((100 - omp_queue_highest_pri) >= wait_set_highest_pri){
        // the priorities of sub-callbacks in omp_queue is higher than that of wait_set
        res = 1;
      }else{
        // the priorities of callbacks in wait_set is higher than that of omp_queue
        if (any_executable.timer) {
          any_executable.timer->call();
        }else if(any_executable.waitable){
          any_executable.data = any_executable.waitable->take_data();
...
// Use mutex instead of wait_mutex_
...
Executor::get_next_executable(AnyExecutable & any_executable, std::chrono::nanoseconds timeout)
{
  ...
  uint64_t start1 = get_clocktime();
  bool success = false;

  pthread_mutex_lock(&mutex);
#if(OMP_TYPE != ROSOMP)
  #if(EXECUTE_TYPE == ROS2_DEFAULT)
  // D-NP or D-GOMP
  success = get_next_ready_executable(any_executable);
  #elif(EXECUTE_TYPE == PICAS_MUTIL)
  // C-NP or C-GOMP
  success = get_next_ready_executable_picas(any_executable);
  #endif
  // If there are none ready callbacks in wait_Set
  if (!success) {
...
#elif(EXECUTE_TYPE == ROS2_DEFAULT)
  // D-ROSOMP
  int res = Call_Comparison();
  if(res == 0){
    // D-ROSOMP
    // the highest-priority callback exists in wait_set
    success = get_next_ready_executable(any_executable);
    if(!success){
      omp_set_wait_for_work_flag(true);
      wait_for_work(timeout);
      omp_set_wait_for_work_flag(false);
    }
    pthread_mutex_unlock(&mutex);
  }else{
    // omp_queue is non-empty and the highest-priority callback exists in omp_queue
    pthread_mutex_unlock(&mutex);
  ...
  // C-ROSOMP
  // ROSOMP Function
  omp_set_wait_for_work_flag_picas(true);
  // When wait_set is empty, the thread does not fall into sleep state
  wait_for_work(timeout);
  omp_set_wait_for_work_flag_picas(false);
  int res = Call_Comparison_picas(any_executable);
  if(res == -1){
    // wait_set is empty and omp_queue is empty
    omp_set_wait_for_work_flag(true);
    // When wait_set is empty, the thread fall into sleep state
    wait_for_work(timeout);
    omp_set_wait_for_work_flag(false);
    pthread_mutex_unlock(&mutex);
  }else if(res == 0){
    // omp_queue is empty or the highest-priority callback exists in wait_set
    success = true;
    pthread_mutex_unlock(&mutex);
  }else{
    // omp_queue is non-empty and the highest-priority callback exists in omp_queue
    pthread_mutex_unlock(&mutex);
    if(omp_get_wait_for_work_flag() == true){
      // Wake up the sleeping executor thread in time
      trigger_interrupt_guard();
    }
    // ROSOMP Function
    omp_dequeue_and_execute();
