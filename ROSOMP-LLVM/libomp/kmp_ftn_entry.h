...
static omp_Node_CP* node_cp = NULL;
static pthread_mutex_t omp_init_Node_CP_lock, omp_init_cnode_lock;

// ROSOMP Functions
// Initialize node_cp
void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_INIT_NODE_CP)(pthread_t key) {
  pthread_mutex_lock(&omp_init_Node_CP_lock);
  if(!node_cp){
    node_cp = (omp_Node_CP*)malloc(sizeof(omp_Node_CP));
    node_cp->key = key;
    node_cp->next = NULL;
    node_cp->node_next = NULL;
  }
  else{
    omp_Node_CP* start = node_cp;
    while(start->next){
      start = start->next;
    }
    start->next = (omp_Node_CP*)malloc(sizeof(omp_Node_CP));
    start->next->key = key;
    start = start->next;
    start->node_next = NULL;
    start->next = NULL;
  }
  pthread_mutex_unlock(&omp_init_Node_CP_lock);
}

void* FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_FIND_NODE)(pthread_t key) {
  omp_Node_CP* start = node_cp;
  while (start) {
      if (start->key == key) 
          return (void*)start;
...
// Get the number of OpenMP threads corresponding to the executor thread
int FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_GET_PTHREAD_NUM)(pthread_t key) {
  omp_Node_CP* start = node_cp;
  while(start){
    if(start->key == key) 
      break;
  ...
  omp_Node_CP* node = start;
  omp_cnode*  temp = node->node_next;
  int num = 0;
  while(temp){
    num++;
    temp = temp->next;
...
// Get the OpenMP thread tid corresponding to the executor thread
pthread_t FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_GET_PTHREAD_TID)(pthread_t key,int num) {
  omp_Node_CP* start = node_cp;
  while(start){
    if(start->key == key) 
      break;
...
// Initialize an OpenMP thread tid corresponding to the executor thread
void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_INIT_CNODE)(pthread_t data, pthread_t key) {
  pthread_mutex_lock(&omp_init_cnode_lock);
  omp_Node_CP* start1 = node_cp;
  while(start1){
    if(start1->key == key) 
      break;
    start1 = start1->next;
  }
  omp_Node_CP* start = start1;
  if(!start->node_next){
    start->node_next =(omp_cnode*)malloc(sizeof(omp_cnode));
    start->node_next->value = data;
    start->node_next->next = NULL;
  } else{
    omp_cnode* temp = start->node_next;
    while(temp->next){
      temp = temp->next;
    }
    temp->next = (omp_cnode*)malloc(sizeof(omp_cnode));
    temp->next->value = data;
    temp-> next->next = NULL;
  }
  pthread_mutex_unlock(&omp_init_cnode_lock);
}

void FTN_STDCALL KMP_EXPAND_NAME(FIN_OMP_SET_STRATEGY)(int temp) {
  omp_set_strategy_kmp(temp);
}

void FTN_STDCALL KMP_EXPAND_NAME(FIN_OMP_SET_EXECUTOR_THREAD_NUM)(int temp){
  omp_set_executor_thread_num_kmp(temp);
}

void FTN_STDCALL KMP_EXPAND_NAME(FIN_OMP_SET_TID_MAP)(long int tid, int executor_id){
  omp_set_tid_map_kmp(tid, executor_id);
}

void FTN_STDCALL KMP_EXPAND_NAME(FIN_OMP_SET_PRIORITY)(int executor_id, long int pri){
  omp_set_priority_kmp(executor_id, pri);
}

int FTN_STDCALL KMP_EXPAND_NAME(FIN_OMP_GET_PRIORITY)(){
  return omp_get_priority_kmp();
}

void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_SYNCHRONIZATION)(int priority){
  omp_synchronization_kmp(priority);
}

void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_SET_WAIT_FOR_WORK_FLAG)(bool flag){
  omp_set_wait_for_work_flag_kmp(flag);
}
bool FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_GET_WAIT_FOR_WORK_FLAG)(){
  return omp_get_wait_for_work_flag_kmp();
}
void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_SET_WAIT_FOR_WORK_FLAG_PICAS)(bool flag){
  omp_set_wait_for_work_flag_picas_kmp(flag);
}
bool FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_GET_WAIT_FOR_WORK_FLAG_PICAS)(){
  return omp_get_wait_for_work_flag_picas_kmp();
}

void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_QUEUE_INIT)(){
  omp_queue_init_kmp();
}

int FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_QUEUE_EMPTY_CHECK)(){
  return omp_queue_empty_check_kmp();
}

void FTN_STDCALL KMP_EXPAND_NAME(FTN_OMP_DEQUEUE_AND_EXECUTE)(){
  omp_dequeue_and_execute_kmp();
}
...
// ROSOMP Functions
KMP_VERSION_SYMBOL(FTN_OMP_INIT_NODE_CP, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_GET_PTHREAD_NUM, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_GET_PTHREAD_TID, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_INIT_CNODE, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_FIND_NODE, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FIN_OMP_SET_STRATEGY, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FIN_OMP_SET_EXECUTOR_THREAD_NUM, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FIN_OMP_SET_TID_MAP, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FIN_OMP_SET_PRIORITY, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FIN_OMP_GET_PRIORITY, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_SYNCHRONIZATION, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_SET_WAIT_FOR_WORK_FLAG, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_GET_WAIT_FOR_WORK_FLAG, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_SET_WAIT_FOR_WORK_FLAG_PICAS, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_GET_WAIT_FOR_WORK_FLAG_PICAS, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_QUEUE_INIT, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_QUEUE_EMPTY_CHECK, 10, "OMP_1.0");
KMP_VERSION_SYMBOL(FTN_OMP_DEQUEUE_AND_EXECUTE, 10, "OMP_1.0");
