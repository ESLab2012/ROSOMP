...
// ROSOMP Functions
// argv_num must be consistent with the number of external parameters within the parallel region!!!
#define argv_num 2
typedef struct omp_Node {
  ...
  pthread_cond_t cond;
  // Doubly Linked List
  struct omp_Node* volatile pre;
  struct omp_Node* volatile next;
} omp_Node;
...
int strategy = -1;
void omp_set_strategy_kmp(int temp){
  strategy = temp;
}

int executor_num = -1;
void omp_set_executor_thread_num_kmp(int temp){
  executor_num = temp;
}
...
void omp_set_tid_map_kmp(long int tid, int executor_id){
  INSERT(tid, executor_id);
}

#define max_executor_num  10
int priority[max_executor_num];
void omp_set_priority_kmp(int executor_id, long int pri){
  priority[executor_id] = pri;
}

int omp_get_priority_kmp(){
  long int Tid = syscall(SYS_gettid);
  int executor_id;
  FIND(Tid, executor_id);
  if(executor_id == -1){
...
void omp_synchronization_kmp(int priority){
...
static omp_Node *queue;
#define node_pool_num 100
pthread_mutex_t pool_lock;
omp_Node* node_pool;

bool wait_for_work_flag;
void omp_set_wait_for_work_flag_kmp(bool flag){
  wait_for_work_flag = flag;
}

bool omp_get_wait_for_work_flag_kmp(){
  return wait_for_work_flag;
}

bool wait_for_work_flag_picas;
void omp_set_wait_for_work_flag_picas_kmp(bool flag){
  wait_for_work_flag_picas = flag;
}

bool omp_get_wait_for_work_flag_picas_kmp(){
  return wait_for_work_flag_picas;
}

void enqueue(kmpc_micro microtask, kmp_int32 argc, void **argv, int priority, int executor_num) {
  pthread_mutex_lock(&pri_count_lock);
  ...
  for(int i = 0;i < executor_num;++i){
    ...
    temp->microtask = microtask;
    temp->argc = argc;
    void **argv_temp = argv_init;
    for(int i = 0;i < argv_num;++i){
      temp->argv[i] = *argv_temp++;
    }
    temp->priority = priority;
...
void omp_queue_init_kmp(){
  ...
  wait_for_work_flag_picas = false;
}

int omp_queue_empty_check_kmp(){
  int result;
  pthread_mutex_lock(&queue_lock);
  if(queue->pre == queue->next && queue->pre == NULL) {
    result = -1;
  }else{
    result = queue->next->priority;
  }
  pthread_mutex_unlock(&queue_lock);
  return result;
}

omp_Node* omp_dequeue() {
  pthread_mutex_lock(&queue_lock);
  while (queue->pre == queue->next && queue->pre == NULL) {
    pthread_mutex_unlock(&queue_lock);
    return NULL;
  }
  // The smaller the value of priority, the higher the priority of the omp sub-callback
...
void omp_dequeue_fin(omp_Node *temp){
  pthread_mutex_lock(&pri_count_lock);
  ...
  if(pre->next == NULL){
    ...
    if(pre->next->count == 0){
      ...
      pre->next = temp->next;
      free(temp);
      // Omp sub-callback synchronization
...
void omp_dequeue_and_execute_kmp(){
  ...
  void *exit_frame_p;
  __kmp_invoke_microtask((VOLATILE_CAST(microtask_t)(temp->microtask)), 0, 0, temp->argc, temp->argv, &exit_frame_p);
...
void __kmpc_fork_call(ident_t *loc, kmp_int32 argc, kmpc_micro microtask, ...) {
  ...
  if(strategy == 2){
    ...
    for (int i = argc - 1; i >= 0; --i)
        *argv++ = va_arg(kmp_va_deref(kmp_va_addr_of(ap)), void *);
    void **argv_temp = args;
    enqueue(microtask, argc, argv_temp, pri, executor_num);
    return;
  }
