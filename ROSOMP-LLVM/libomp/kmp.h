...
typedef struct omp_cnode{
  pthread_t value;
  struct  omp_cnode* next;
}omp_cnode;

typedef struct omp_Node_CP{
  pthread_t key;
  omp_cnode* node_next;
  struct omp_Node_CP* next;
}omp_Node_CP;

// ROSOMP Functions
KMP_EXPORT void omp_set_strategy_kmp(int temp);
KMP_EXPORT void omp_set_executor_thread_num_kmp(int temp);
KMP_EXPORT void omp_set_tid_map_kmp(long int tid, int executor_id);
KMP_EXPORT int omp_get_priority_kmp();
KMP_EXPORT void omp_set_priority_kmp(int executor_id, long int pri);
KMP_EXPORT void omp_synchronization_kmp(int priority);
KMP_EXPORT void omp_set_wait_for_work_flag_kmp(bool flag);
KMP_EXPORT bool omp_get_wait_for_work_flag_kmp();
KMP_EXPORT void omp_set_wait_for_work_flag_picas_kmp(bool flag);
KMP_EXPORT bool omp_get_wait_for_work_flag_picas_kmp();
KMP_EXPORT void omp_queue_init_kmp();
KMP_EXPORT int omp_queue_empty_check_kmp();
KMP_EXPORT void omp_dequeue_and_execute_kmp();
