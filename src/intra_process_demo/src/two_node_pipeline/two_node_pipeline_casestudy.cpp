...
int main(int argc, char * argv[])
{
  ...
  auto Sub1 = std::make_shared<Command>("Sub1", "CHAIN1_TIMER_OUT", 1, 2, 201, OMP_TYPE == CLOSE ? false : true, executor); // first omp callback
  executor.add_node(Sub1);

  auto Transfer2_1 = std::make_shared<Transfer>("Transfer2_1", "CHAIN1_TIMER_OUT", "CHAIN2_Transfer_OUT", 2, 2, 291, OMP_TYPE == CLOSE ? false : true, executor); // second omp callback
  executor.add_node(Transfer2_1);

  auto Sub2 = std::make_shared<Command>("Sub2", "CHAIN2_Transfer_OUT", 2, 3, 42, false, executor);
  executor.add_node(Sub2);

  auto Transfer3_1 = std::make_shared<Transfer>("Transfer3_1", "CHAIN3_TIMER_OUT", "CHAIN3_Transfer_OUT", 3, 2, 210, OMP_TYPE == CLOSE ? false : true, executor); // third omp callback
  ...
  auto Sub3 = std::make_shared<Command>("Sub3", "CHAIN3_SUB_OUT", 3, 4, 37, false, executor); 
  executor.add_node(Sub3);

  auto Transfer4 = std::make_shared<Transfer>("Transfer4_1", "CHAIN4_TIMER_OUT", "CHAIN4_SUB_OUT", 4, 2, 24, false, executor);
  ...
  auto Transfer5 = std::make_shared<Transfer>("Transfer5", "CHAIN5_TIMER_OUT", "CHAIN5_SUB_OUT", 5, 2, 23, false, executor);
  executor.add_node(Transfer5);

  auto Sub5 = std::make_shared<Command>("Sub5", "CHAIN5_SUB_OUT", 5, 3, 26, false, executor);
  executor.add_node(Sub5);

  auto Sub6 = std::make_shared<Command>("Sub6", "CHAIN6_TIMER_OUT", 6, 2, 31, false, executor);
  executor.add_node(Sub6);

  auto Sub7 = std::make_shared<Command>("Sub7", "CHAIN7_TIMER_OUT", 7, 2, 37, false, executor);
  executor.add_node(Sub7);

  auto Timer1 = std::make_shared<Sensor>("Timer1", "CHAIN1_TIMER_OUT", 1, 1, 30, 50ms, false, executor);
  executor.add_node(Timer1);

  auto Timer3 = std::make_shared<Sensor>("Timer3", "CHAIN3_TIMER_OUT", 3, 1, 34, 100ms, false, executor);
  executor.add_node(Timer3);

  auto Timer4 = std::make_shared<Sensor>("Timer4", "CHAIN4_TIMER_OUT", 4, 1, 42, 100ms, false, executor);
  executor.add_node(Timer4);

  auto Timer5 = std::make_shared<Sensor>("Timer5", "CHAIN5_TIMER_OUT", 5, 1, 127, 100ms, false, executor);
  executor.add_node(Timer5);

  auto Timer6 = std::make_shared<Sensor>("Timer6", "CHAIN6_TIMER_OUT", 6, 1, 134, 150ms, false, executor);
  executor.add_node(Timer6);

  auto Timer7 = std::make_shared<Sensor>("Timer7", "CHAIN7_TIMER_OUT", 7, 1, 139, 200ms, false, executor);
